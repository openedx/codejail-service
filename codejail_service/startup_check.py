...
def _check_network_access():
    ...
    # into those problems.)
    #
    # We go to the trouble of properly closing the socket because there's a unit
    # test where the socket is successfully created, and if we don't close it
    # then there's a ResourceWarning, which causes a `__warningregistry__`
    # global to be created. This would interfere with checking the logged error
    # message, which includes a listing of globals.
    (globals_out, error_message) = safe_exec(
        dedent("""
          import socket
